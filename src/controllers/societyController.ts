...
export const getAllSocieties = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: 'success', results: societies.length, data: { societies } });
  } catch (err) {
    return next(err);
  }
};
...
export const assignCabinetMember = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!societyId || !userId || !positionId) {
      return res
        .status(400)
        .json({ status: 'fail', message: 'Please provide societyId, userId, and positionId' });
    }
    ...
    if (memberPosition && Number(positionId) === memberPosition.id) {
      return res
        .status(400)
        .json({ status: 'fail', message: 'Use joinsociety for regular members.' });
    }
    ...
    return res.status(201).json({
      status: 'success',
      message: 'Cabinet member assigned successfully',
      data: { member },
    });
  } catch (err: any) {
    if (err.code === 'P2002') {
      ...
      if (existing) {
        ...
        return res.status(200).json({
          status: 'success',
          message: 'Member position updated',
          data: { member: updated },
        });
      }
    }
  ...
  try {
    ...
    if (!memberPosition) {
      return res.status(500).json({
        status: 'error',
        message: "Default 'Member' position not found in database. Please contact Admin.",
      });
    }
    ...
    if (err.code === 'P2002')
      return res
        .status(400)
        .json({ status: 'fail', message: 'You are already a member of this society' });
    return next(err);
  }
};
...
export const leaveSociety = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!membership) {
      return res
        .status(404)
        .json({ status: 'fail', message: 'You are not a member of this society' });
    }
    ...
    return res
      .status(200)
      .json({ status: 'success', message: 'You have successfully left the society' });
  } catch (err) {
    return next(err);
  }
};
...
export const removeMember = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!societyId || !userId) {
      return res
        .status(400)
        .json({ status: 'fail', message: 'Please provide societyId and userId' });
    }
    ...
    if (!isAdmin && !isIncharge) {
      return res
        .status(403)
        .json({ status: 'fail', message: 'Unauthorized to remove members from this society' });
    }
    ...
    if (!membership) {
      return res
        .status(404)
        .json({ status: 'fail', message: 'User is not a member of this society' });
    }
...
export const getMyJoinedSocieties = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: 'success', results: memberships.length, data: { memberships } });
  } catch (err) {
    return next(err);
  }
};
...
export const deleteEvent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res
      .status(204)
      .json({ status: 'success', data: null, message: 'Event deleted successfully' });
  } catch (err) {
    return next(err);
  }
};
